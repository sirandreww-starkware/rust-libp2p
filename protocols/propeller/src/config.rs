...
/// The types of message validation that can be employed by turbine.
...
pub struct Config {
    ...
    /// Time to keep previous leaders in cache to discard late-arriving shreds.
    previous_leaders_ttl: Duration,
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            previous_leaders_ttl: Duration::from_secs(10),
            reconstructed_messages_ttl: Duration::from_secs(60),
...
impl Config {
    ...
    /// Time to keep previous leaders in cache.
    pub fn previous_leaders_ttl(&self) -> Duration {
        self.previous_leaders_ttl
    }
...
impl ConfigBuilder {
    /// Set the data plane fanout (number of peers each node forwards shreds to).
    pub fn data_plane_fanout(mut self, fanout: usize) -> Self {
    ...
    /// Set the previous leaders TTL.
    pub fn previous_leaders_ttl(mut self, ttl: Duration) -> Self {
        self.config.previous_leaders_ttl = ttl;
        self
    }
