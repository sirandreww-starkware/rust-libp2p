...
use crate::{
    ...
    message::{MessageId, PropellerMessage, Shred, ShredHash, ShredId, ShredIndex},
    signature,
    tree::PropellerTreeManager,
    types::{Event, PropellerError, ShredVerificationError},
    ValidationMode,
};
...
pub struct Behaviour {
    ...
    /// Map of peer IDs to their weights for tree topology calculation.
    ...
    verified_shreds: HashMap<MessageId, HashMap<ShredIndex, Shred>>,

    /// Cache of message ids for which messages have already been reconstructed and emitted.
    reconstructed_messages: lru_time_cache::LruCache<MessageId, ()>,

    /// Cache for previous leaders to discard late-arriving shreds during leader transitions.
    previous_leaders: lru_time_cache::LruCache<PeerId, ()>,
}
...
impl Behaviour {
    ...
    pub fn new(message_authenticity: MessageAuthenticity, config: Config) -> Self {
        ...
        let previous_leaders =
            lru_time_cache::LruCache::with_expiry_duration(config.previous_leaders_ttl());
        ...
        Self {
            ...
            verified_shreds: HashMap::new(),
            reconstructed_messages,
            previous_leaders,
        }
    }

    /// Add a peer with its weight for tree topology calculation.
    /// Does not require existing connection - will be used when connection is established.
    ///
    /// Optionally, if a public key is provided, it will be stored for signature verification.
    /// If no public key is provided, the method will attempt to extract it from the PeerId
    /// (this only works for small keys like Ed25519 that are embedded in the PeerId).
    pub fn add_peers(
        &mut self,
        peers: impl IntoIterator<Item = (PeerId, u64)>,
    ) -> Result<(), PropellerError> {
        for (peer_id, weight) in peers {
            self.add_peer_with_key(peer_id, weight, None)?;
        }
        self.tree_manager.update_nodes(&self.peer_weights)?;
        Ok(())
    }

    pub fn add_peers_with_keys(
        &mut self,
        peers: impl IntoIterator<Item = (PeerId, u64, PublicKey)>,
    ) -> Result<(), PropellerError> {
        for (peer_id, weight, public_key) in peers {
            self.add_peer_with_key(peer_id, weight, Some(public_key))?;
        }
        self.tree_manager.update_nodes(&self.peer_weights)?;
        Ok(())
    }
    ...
    fn add_peer_with_key(
        ...
        if let Some(public_key) = public_key {
            if signature::validate_public_key_matches_peer_id(&public_key, &peer_id) {
                ...
                return Err(PropellerError::InvalidPublicKey);
            }
    ...
    /// Remove a peer and its weight from the tree topology calculation.
    pub fn remove_peer(&mut self, peer_id: &PeerId) -> Result<(), PropellerError> {
        if self.peer_weights.remove(peer_id).is_some() {
            debug!(peer=%peer_id, "Removed peer from weight map");

            // Also remove the public key if we have it
            if self.peer_public_keys.remove(peer_id).is_some() {
                debug!(peer=%peer_id, "Removed peer public key");
            }

            // Update the tree manager with new peer weights
            self.tree_manager.update_nodes(&self.peer_weights)?;
        }
        Ok(())
    }
    ...
    pub fn peer_count(&self) -> usize {
        self.peer_weights.len()
    }

    /// Set the current leader peer ID.
    /// Leader will broadcast shreds, non-leaders will retransmit.
    ///
    /// Note: Changing the leader invalidates the current tree topology,
    /// so all cached shreds are cleared to prevent stale data.
    pub fn set_leader(&mut self, leader_id: PeerId) -> Result<(), PropellerError> {
        if Some(leader_id) == self.tree_manager.current_leader() {
            return Ok(());
        }

        // Add current leader to previous leaders cache before changing
        if let Some(current_leader) = self.tree_manager.current_leader() {
            self.previous_leaders.insert(current_leader, ());
            self.previous_leaders.remove(&leader_id);
        }

        // Clear all cached data when leader changes since tree topology changes
        self.verified_shreds.clear();
        self.tree_manager.set_leader(leader_id)
    }

    /// Get the current leader peer ID.
    pub fn current_leader(&self) -> Option<PeerId> {
        self.tree_manager.current_leader()
    }

    /// Check if this node is currently the leader.
    pub fn is_leader(&self) -> bool {
        self.tree_manager.is_leader()
    }

    /// Broadcast data as shreds (only works if this node is the leader).
    /// The data will be split into fec_data_shreds equal parts. The data size must be
    /// divisible by fec_data_shreds.
    pub fn broadcast(
        &mut self,
        data: Vec<u8>,
        ...
        if !self.is_leader() {
            return Err(PropellerError::NotLeader);
        }
        ...
        if data.len() % num_data_shreds != 0 {
            return Err(PropellerError::InvalidDataSize);
        }

        // Create shreds from data
        let shreds = self.create_shreds_from_data(data, message_id)?;

        // Send shreds to root nodes in the propeller tree
        for shred in shreds {
            let shred_hash = shred.hash_without_signature();
            self.broadcast_shred_to_children_in_tree(shred, &shred_hash)?;
        }
    ...
    /// Add a peer to the connected peers set for testing purposes.
    ///
    /// This method is intended for unit tests where you need to simulate
    /// peer connections without establishing actual network connections.
    pub fn add_connected_peer_for_test(&mut self, peer_id: PeerId) {
    ...
    fn verify_shred_signature(&self, shred: &Shred) -> Result<(), PropellerError> {
        ...
        let Some(signer_public_key) = self.peer_public_keys.get(&shred.id.publisher) else {
            return Err(PropellerError::ShredSignatureVerificationFailed(format!(
                "No public key available for signer {}",
                shred.id.publisher
            )));
        };
    ...
    fn sign_shred(&self, shred: &Shred) -> Result<Vec<u8>, PropellerError> {
        signature::sign_shred(shred, &self.message_authenticity)
    }

    /// Validate a received shred from a peer.
    /// Returns Ok(shred_hash) if validation passes, or Err(ShredVerificationError) if validation
    /// fails.
    pub fn validate_shred(
        &self,
        peer_id: PeerId,
        shred: &Shred,
    ) -> Result<ShredHash, ShredVerificationError> {
        let Some(leader_peer_id) = self.tree_manager.current_leader() else {
            return Err(ShredVerificationError::NoLeaderSet);
        };

        if shred.id.publisher != leader_peer_id {
            return Err(ShredVerificationError::UnexpectedPublisher {
                publisher: shred.id.publisher,
            });
        }

        if leader_peer_id == self.tree_manager.get_local_peer_id() {
            return Err(ShredVerificationError::LeaderReceivingShred);
        }

        if let Some(message_shreds) = self.verified_shreds.get(&shred.id.message_id) {
            if message_shreds.contains_key(&shred.id.index) {
                return Err(ShredVerificationError::DuplicateShred);
            }
        }

        let shred_hash = shred.hash_without_signature();

        let expected_parent = match self.tree_manager.get_parent_in_tree(&shred_hash) {
            Ok(Some(parent)) => parent,
            Ok(None) => {
                unreachable!("Local node is the leader, this is not possible at this point")
            }
            Err(e) => {
                return Err(ShredVerificationError::TreeError(e));
            }
        };

        if expected_parent != peer_id {
            return Err(ShredVerificationError::ParentVerificationFailed {
                expected_sender: expected_parent,
            });
        }

        if let Err(e) = self.verify_shred_signature(shred) {
            return Err(ShredVerificationError::SignatureVerificationFailed(e));
        }
    ...
    fn handle_received_shred(&mut self, peer_id: PeerId, shred: Shred) {
        if self.previous_leaders.contains_key(&shred.id.publisher) {
            return;
        }

        let shred_hash = match self.validate_shred(peer_id, &shred) {
            Ok(hash) => hash,
            Err(error) => {
                self.emit_event(Event::ShredVerificationFailed { peer_id, error });
                return;
            }
        };

        let message_shreds = self.verified_shreds.entry(shred.id.message_id).or_default();
        ...
        if let Err(e) = self.broadcast_shred_to_children_in_tree(shred, &shred_hash) {
            self.emit_event(Event::ShredSendFailed {
                peer_id,
                error: format!("Failed to forward shred to children in tree: {}", e),
            });
        }

        // Check if we have enough shreds to reconstruct the message
        self.try_reconstruct_message(message_id);
    }

    /// Try to reconstruct the original message from verified shreds for a given message id.
    fn try_reconstruct_message(&mut self, message_id: MessageId) {
        // Check if we've already reconstructed and emitted this message
        if self.reconstructed_messages.contains_key(&message_id) {
            return;
        }

        let message_shreds = self.verified_shreds.get(&message_id).unwrap();
    ...
    pub fn create_shreds_from_data(
        ...
        data: Vec<u8>,
        message_id: MessageId,
    ) -> Result<Vec<Shred>, PropellerError> {
        let num_data_shreds = self.config.fec_data_shreds();
    ...
    fn generate_coding_shreds(&self, data_shreds: &[Shred]) -> Result<Vec<Shred>, PropellerError> {
        ...
        let shred_size = data_shreds
            .first()
            .ok_or(PropellerError::ErasureEncodingFailed(
        ...
        let mut encoder =
            ReedSolomonEncoder::new(data_count, coding_count, shred_size).map_err(|e| {
                PropellerError::ErasureEncodingFailed(format!(
        ...
        for shred in data_shreds.iter().take(data_count) {
            encoder.add_original_shard(&shred.data).map_err(|e| {
                PropellerError::ErasureEncodingFailed(format!("Failed to add data shred: {}", e))
            })?;
        }

        // Perform Reed-Solomon encoding
        let result = encoder.encode().map_err(|e| {
            PropellerError::ErasureEncodingFailed(format!("Failed to encode: {}", e))
        })?;
    ...
    fn reconstruct_message_from_shreds(
        ...
        let shred_size = shreds
            .first()
            .ok_or(PropellerError::ErasureDecodingFailed(
                "No shreds".to_string(),
            ))?
            .data
            .len();
        ...
        let mut decoder =
            ReedSolomonDecoder::new(data_count, coding_count, shred_size).map_err(|e| {
                PropellerError::ErasureDecodingFailed(format!(
                    "Failed to create Reed-Solomon decoder: {}",
                    e
                ))
            })?;

        // Create a mapping of shred index to data for efficient lookup
        let mut shred_map = std::collections::HashMap::new();
        for shred in shreds {
            shred_map.insert(shred.id.index, &shred.data);
        }
        ...
        let result = decoder.decode().map_err(|e| {
            PropellerError::ErasureDecodingFailed(format!("Failed to decode: {}", e))
        })?;
        ...
        for index in 0..data_count {
            if let Some(shred_data) = shred_map.get(&(index as u32)) {
                // We have the original shred data
                reconstructed_data.extend_from_slice(shred_data);
            } else {
                // We need to get the restored data from Reed-Solomon
                    ...
                    .map(|(_, data)| data)
                {
                    ...
                    return Err(PropellerError::ErasureDecodingFailed(format!(
    ...
    fn broadcast_shred_to_children_in_tree(
        &mut self,
        shred: Shred,
        shred_hash: &ShredHash,
    ) -> Result<(), PropellerError> {
        // Get broadcast peer (root node) for this shred
        let children = self.tree_manager.get_children_in_tree(shred_hash)?;
        for child in children {
            if !self.connected_peers.contains(&child) {
                return Err(PropellerError::PeerNotConnected(child));
            }
            let message = PropellerMessage {
                shred: shred.clone(),
            };
            self.emit_handler_event(child, HandlerIn::SendMessage(message));
        }
        Ok(())
    }
...
impl NetworkBehaviour for Behaviour {
    ...
    fn handle_established_inbound_connection(
        ...
        Ok(Handler::new(self.config.stream_protocol().clone()))
    }
    ...
    fn on_connection_handler_event(
        ...
        match event {
            ...
            HandlerOut::SendError(error) => {
                self.emit_event(Event::ShredSendFailed { peer_id, error });
            }
        }
    }
