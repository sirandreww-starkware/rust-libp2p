...
use crate::{behaviour::MessageAuthenticity, message::Shred, types::PropellerError};
    ...
    match message_authenticity {
        MessageAuthenticity::Signed(keypair) => {
            ...
            match keypair.sign(&signature_bytes) {
                ...
                Err(e) => Err(PropellerError::SigningFailed(e.to_string())),
            }
        }
...
pub(crate) fn verify_shred_signature(
    ...
    if shred.signature.is_empty() {
        return Err(PropellerError::ShredSignatureVerificationFailed(
            "Shred has empty signature".to_string(),
        ));
    }
    let shred_bytes = shred.encode_without_signature();
    let mut signature_bytes = SIGNING_PREFIX.to_vec();
    ...
    if signature_valid {
        ...
        Err(PropellerError::ShredSignatureVerificationFailed(
            "Shred signature is invalid".to_string(),
        ))
    }
}
