...
//! leader egress bandwidth while ensuring rapid and resilient block propagation
...
//!    infrastructure.
//!
//! 3. **Publisher as Root**: The message publisher is always the root of every tree, simplifying
//!    the topology and eliminating the need for separate root node selection.
//!
//! ## Key Features
//!
...
//!     .data_plane_fanout(100) // Fanout of 100
//!     .build();
//!
...
//! propeller
//!     .add_peers(vec![(local_peer_id, 2000), (peer1, 1000), (peer2, 500)])
//!     .unwrap();
//!
//! // Set leader
//! let leader_peer_id = peer1; // Use one of the peers we added
//! propeller.set_leader(leader_peer_id).unwrap();
//!
//! // Broadcast data (if this node is leader)
...
pub use self::{
    ...
    types::{Event, PropellerError, PropellerNode, ShredVerificationError},
};
