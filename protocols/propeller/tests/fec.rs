...
fn test_reed_solomon_fec_generation() {
    ...
    let _ = turbine.set_leader(local_peer);
    ...
    match turbine.create_shreds_from_data(test_data.clone(), 0) {
        Ok(all_shreds) => {
            ...
            let coding_data_is_real = coding_shreds
                .iter()
                .any(|shred| shred.data.iter().any(|&byte| byte != 0));
            ...
            if coding_data_is_real {
                ...
                tracing::warn!("⚠️  Coding shreds are all zeros - may indicate issue with encoding");
            }
...
fn test_fec_disabled() {
    ...
    // Local peer ID is now set automatically in constructor
    turbine.add_peers(vec![(local_peer, 1000)]).unwrap();
    turbine.set_leader(local_peer).unwrap();
