...
fn test_propeller_behaviour_creation() {
    ...
    let behaviour = Behaviour::new(message_authenticity, config);

    assert!(!behaviour.is_leader());
    assert_eq!(behaviour.current_leader(), None);
}
...
fn test_peer_management() {
    ...
    let _ = behaviour.add_peers(vec![(peer1, 1000), (peer2, 500)]);

    // Remove a peer
    let _ = behaviour.remove_peer(&peer1);
}
...
fn test_leader_management() {
    ...
    behaviour
        .add_peers(vec![(local_peer_id, 1500), (leader_id, 1000)])
        .unwrap();

    // Set leader
    behaviour.set_leader(leader_id).unwrap();
    assert_eq!(behaviour.current_leader(), Some(leader_id));
}
...
fn test_signature_verification() {
    ...
    let _ = behaviour.add_peers_with_keys(vec![(our_peer_id, 1000, keypair.public())]);

    // Now set as leader
    let _ = behaviour.set_leader(our_peer_id);
    assert!(behaviour.is_leader());
...
fn test_peer_public_key_extraction() {
    ...
    let _ = behaviour.add_peers(vec![(peer_id, 1000)]);

    // Test 2: Add peer with explicit public key
    let keypair2 = Keypair::generate_ed25519();
    let peer_id2 = PeerId::from(keypair2.public());
    let _ = behaviour.add_peers_with_keys(vec![(peer_id2, 500, keypair2.public())]);

    // Test 3: Try to add a random PeerId (won't have extractable key)
    let random_peer = PeerId::random();
    let _ = behaviour.add_peers(vec![(random_peer, 250)]);
...
fn test_key_validation_security() {
    ...
    let _ = behaviour.add_peers_with_keys(vec![(peer_id1, 1000, keypair1.public())]);
    ...
    let _ = behaviour.add_peers_with_keys(vec![(different_peer_id, 500, keypair2.public())]);
