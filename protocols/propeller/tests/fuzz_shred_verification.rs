...
use libp2p_propeller::{Behaviour, Config, MessageAuthenticity, ShredVerificationError};
...
fn create_fuzz_test_setup() -> FuzzTestSetup {
    ...
    leader_behaviour
        .add_peers(vec![(leader_peer_id, 2000), (follower_peer_id, 1000)])
        .unwrap();
    follower_behaviour
        .add_peers(vec![(leader_peer_id, 2000), (follower_peer_id, 1000)])
        .unwrap();

    // Set leader for both behaviours
    leader_behaviour.set_leader(leader_peer_id).unwrap();
    follower_behaviour.set_leader(leader_peer_id).unwrap();
...
fn test_deterministic_shred_corruption_fuzzing() {
    ...
    let original_shreds_encoded = shreds
        ...
        .map(|shred| {
            ...
            shred.encode(&mut valid_shred_bytes);
    ...
    for seed in 0..FUZZ_ITERATIONS {
        ...
        let Some(corrupted_shred) = libp2p_propeller::Shred::decode(&mut corrupted_bytes_mut)
        else {
            continue;
        };
        ...
        let behaviour = if seed % 51 == 0 {
            &setup.leader_behaviour
        } else {
            &setup.follower_behaviour
        };
        match behaviour.validate_shred(sender, &corrupted_shred) {
            ...
            Err(error) => {
                error_counter[match error {
                    ShredVerificationError::NoLeaderSet => 0,
                    ShredVerificationError::UnexpectedPublisher { .. } => 1,
                    ShredVerificationError::LeaderReceivingShred => 2,
                    ShredVerificationError::DuplicateShred => 3,
                    ShredVerificationError::TreeError(_) => 4,
                    ShredVerificationError::ParentVerificationFailed { .. } => 5,
                    ShredVerificationError::SignatureVerificationFailed(_) => 6,
                }] += 1;
            }
        }
    }
