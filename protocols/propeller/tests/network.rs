...
async fn test_turbine_message_propagation_demo() {
    ...
    let _ = node1
        .behaviour_mut()
        .add_peers(vec![(peer1, 1500), (peer2, 1000)]);
    let _ = node2
        .behaviour_mut()
        .add_peers(vec![(peer1, 1500), (peer2, 1000)]);
    ...
    tracing::info!("Node1 is leader: {}", node1.behaviour().is_leader());
    ...
    for round in 0..20 {
        ...
        if let Some(event) = node2.next_behaviour_event().now_or_never() {
            match event {
                Event::ShredReceived { peer_id, shred } => {
                    tracing::info!(
...
fn test_turbine_api_with_100_peers_fanout_6() {
    ...
    let peers_to_add: Vec<(PeerId, u64)> = peers
        ...
        .cloned()
        .collect();
    let _ = turbine.add_peers(peers_to_add);

    tracing::info!("✅ Added {} peers to turbine behaviour", NUM_PEERS - 1);

    // Test different leader scenarios
    for test_round in 0..5 {
        let leader_index = test_round * 20; // Pick different leaders
        let leader_peer_id = peers[leader_index].0;

        let _ = turbine.set_leader(leader_peer_id);

        let is_leader = turbine.is_leader();
        tracing::info!(
            "Round {}: Leader={} (index {}), We are leader: {}",
            test_round,
            leader_peer_id,
            leader_index,
            is_leader
