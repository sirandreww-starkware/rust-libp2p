...
pub(crate) fn generate_random_shred(seed: u64) -> libp2p_propeller::Shred {
    ...
    let data_len = rng.random_range(0..=4096);
    ...
    libp2p_propeller::Shred {
        id: libp2p_propeller::ShredId {
            message_id: topic,
            index,
            publisher,
        },
...
fn test_encode_decode_roundtrip_fuzzing() {
    ...
    for seed in 0..ITERATIONS {
        ...
        original_shred.encode(&mut encoded_bytes);
        let mut decode_buffer = encoded_bytes.clone();
        let Some(decoded_shred) = libp2p_propeller::Shred::decode(&mut decode_buffer) else {
            panic!(
